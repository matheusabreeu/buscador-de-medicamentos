...
async function buscarVTEX(medicamento, loja) {
    try {
        ...
        return data.map(p => {
            ...
            // TRAVA DE SEGURANÇA PARA LINKS DUPLICADOS
...
app.all('*', async (req, res) => {
    ...
    // Montagem segura da lista de resultados
    let listaHTML = '';
    resultados.forEach((r, index) => {
        const corLoja = r.loja === 'Extrafarma' ? 'text-blue-400' : (r.loja === 'Globo' ? 'text-orange-500' : 'text-red-400');
        const badgeMelhorPreco = index === 0 ? '<span class="bg-green-500/20 text-green-400 text-[8px] px-2 py-0.5 rounded-full font-black ml-2">MELHOR PREÇO</span>' : '';
    ...
    <body class="bg-slate-950 text-white p-4 font-sans">
        <div class="max-w-md mx-auto">
            <header class="text-center py-6">
                ...
                <p class="text-slate-500 text-[10px] uppercase tracking-widest mt-1 font-bold">Monitoramento de Preços São Luís | UFMA</p>
            </header>
